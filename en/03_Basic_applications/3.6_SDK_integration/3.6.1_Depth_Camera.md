...
# 3.6.1 Depth Camera Usage Guide

## Overview

This guide provides instructions for using **HBV series stereo cameras**, specifically binocular, monochrome, global-shutter, 720P models commonly available on the market.

![alt text](images/image.png)

## Software Installation

Before using the camera, install **GStreamer**. Follow the official guide here: [GStreamer User Guide](https://www.spacemit.com/community/document/info?lang=en&nodepath=software/SDK/buildroot/media/gstreamer_user_guide.md)

## Usage Guide

**Important:** Always connect the camera to a **USB 3.0 port** for full performance.

### Check Camera Device and Supported Formats

Run the following command to view the camera's supported resolutions and formats:
...
- **Top section (`caps`)**: Lists the supported resolutions and frame rates in both `image/jpeg` and `video/x-raw` formats.
- **Bottom section (`properties`)**: Shows detailed device properties. Note the `api.v4l2.path` value (for example, `/dev/video20`), as this device identifier will be used in subsequent commands.

### Start the Camera

The camera supports two operating modes:

**Mode 1: video/x-raw mode (no decoding, requires USB 3.0)**
...
**Mode 2: image/jpeg mode (includes decoding and does not require USB 3.0)**

```bash
# If the previous run was terminated with Ctrl+C, you may need to run this twice
...
Try running the command again several times, or reconnect the camera.
...
### Using a Camera with OpenCV

**Note:** When building OpenCV, make sure the **WITH_GSTREAMER** option is enabled.
...
int main(int argc, char **argv) {
    signal(SIGINT, signal_handler);  // Capture Ctrl-C signal
...
Stereo camera calibration follows the steps below.

#### Prepare Calibration Board

- Use [this checkerboard generator](https://calib.io/pages/camera-calibration-pattern-generator).
- Set the dimensions to **297 × 210 mm** (standard A4 paper) and print the pattern.
- Recommendation: use more than 6 × 8 squares, with each square wider than 25 mm.

#### Capture Calibration Images

- Capture images at the specified resolution, and use code to separate the left and right images into their respective folders (`left` and `right`).
- **Capture requirements:**
    - Both cameras must capture the full checkerboard.
    - Capture images from different distances and angles.
    - Avoid camera shake; capture images only when the camera is stable.
    - Avoid reflections on the calibration board.

        **Python code for capturing calibration images:**

```python3
    import cv2
    import numpy as np
    import os
    cap = cv2.VideoCapture("/dev/video0")# Use actual device number
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)# Specify according to the required resolution.
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)# Specify according to the required resolution.
...
#### MATLAB Calibration
    ...
    Use the calibration tool in MATLAB to generate the calibration file.
    Example calibration screenshots:
   ![alt text](images/matlab1.png)
   ![alt text](images/matlab2.png)
   ![alt text](images/matlab3.png)
   ![alt text](images/matlab4.png)

    **Important:** Keep the calibration error below **0.1**.

#### Export Calibration File

```matlab
% Extract camera intrinsic matrices
...
disp('YAML file generated: stereo_params.yaml');
```

#### Read Calibration File and Perform Stereo Matching in OpenCV
...
# ------------------------
# CMakeLists.txt
# Put this file in the project root directory
# ~/project/CMakeLists.txt
# ------------------------
cmake_minimum_required(VERSION 3.10)
project(stereo_test)
...
void readYaml(const std::string& filename, cv::Mat &K1, cv::Mat &D1, cv::Mat &K2, cv::Mat &D2, cv::Mat &R, cv::Mat &T) {
    try {
        ...
        // Read rotation matrix R and translation vector T
...
int main(int argc, char **argv){
    ...
    T /= 1000.0; // Convert units (e.g., from mm to meters)
